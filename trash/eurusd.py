...
lightstreamer_client = LSClient("https://dfx.dailyfx.com", "InVisionProvider")
try:
...
subscription = Subscription(
    mode="MERGE",
    items=["MARKET:EURUSD"],
    #fields=["BID","OFFER","HIGH","LOW","CHANGE_PCT"],
    fields=["BID"],
    adapter="")
...
def on_item_update(item_update):
    ...
    with open('eurusd.log','a') as outfile:
        outfile.write(str(datetime.datetime.now())+";"+str(int(round(time.time()*1000)))+";"+str(item_update["values"]["BID"])+"\n")
    #print(str(int(round(time.time()*1000)))+";"+str(item_update["values"]["BID"]))
    #print(str(int(round(time.time()*1000)))+";"+item_update["values"]["BID"])
